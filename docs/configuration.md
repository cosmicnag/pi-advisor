...
| ------------------------ | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Lifecycle state          | Independently of activity recording when append succeeds               | Version, Pi session ID, save time, branch cursor, retained accepted deferred notes, up to 128 dedupe hashes, delivery counts, and Memory suggestion cadence and cap state                                         | Executor reasoning, Advisor reasoning, provider payloads, activity updates, protected tool output, suppressed or rejected notes, and raw failure text                     | New snapshots follow the Pi session. Delete the Pi session file to delete them.           |
...
New activity records use strict transcript schema version `2` and cannot be confused with legacy content-bearing version `1` records.
Valid version `1` records remain readable for bounded diagnostics; malformed and unsupported versions are safely ignored.
