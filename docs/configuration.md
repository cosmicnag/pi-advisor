...
| Optional accepted advice                    | Persistence enabled after accepted queue or delivery state            | Bounded accepted ordinary note or eligible Memory suggestion shape                                                                                                        | Suppressed, duplicate, rejected, oversized, unsafe, redaction-altered proposals, and internal semantic finding hashes                                   | No time expiry. Delete the Pi session file.                                               |
| Optional failure, governor outcome, or stop | Persistence enabled                                                   | Bounded redacted failure reason or handled governor outcome and supported stop metadata                                                                                   | Raw error objects, credentials, and reasoning                                                                                                           | No time expiry. Delete the Pi session file.                                               |

Lifecycle state format version `2` adds the optional opaque semantic finding hash to deferred review advice.
Version `1` lifecycle state remains accepted and is migrated in memory without losing compatible deferred advice, counters, or cadence state.
Version `1` dedupe hashes cannot be recalculated into the new severity-independent identity, so their previously delivered suppression history starts fresh after migration.
Optional transcript records remain at version `1` and omit the internal semantic finding hash, preserving their strict version `1` shape.
Downgrading to a build that only understands lifecycle state version `1` is not supported; that build safely ignores version `2` lifecycle snapshots.
