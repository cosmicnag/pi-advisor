...
Pi Advisor 0.2.1 requires Pi `>=0.81.1 <0.82.0`; Pi Advisor 0.1.3 remains the legacy release for Pi 0.80.7.
...
New activity records continue to use strict transcript schema version `2` and cannot be confused with lifecycle state version `3` or legacy content-bearing transcript version `1` records.
...
Downgrading to a build that does not understand lifecycle state version `3` is not supported; that build safely ignores the newer lifecycle snapshots.
