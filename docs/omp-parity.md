...
| Idle and terminal deferred advice                  | implemented            | Individually bounded notes are held in memory and injected after the next user prompt without triggering a completion                 |
| Cross-update dedupe                                | implemented            | An NFKC, case-, punctuation-, symbol-, and whitespace-normalized 4,096-key FIFO suppresses duplicate notes and clears on branch reset |
