...
| Idle and terminal deferred advice                  | implemented            | Bounded in-memory advice is injected after the next user prompt without triggering a completion         |
| Cross-update dedupe                                | implemented            | A normalized 4,096-key FIFO suppresses duplicate notes and clears on branch reset                       |
| Staleness annotation                               | implemented            | Advice produced after the Executor advances beyond the submitted window is marked potentially stale     |
...
| Deliberate user-interrupt distinction              | intentional difference | Pi exposes no public abort cause, so every aborted turn is skipped and forces pending advice to defer   |
